...
{ //se lance lorsque fenêtre s'affiche
 
    var canvasWidth = 900; //largeur
    var canvasHeight = 600; // hauteur
    var blockSize = 25; //taile des blocks
    var ctx;
    var delay = 100; //temps en ms
    var snakee;
    var applee;
    var widthInBlocks = canvasWidth/blockSize;//divise la largeur en terme de blocks donc 30 blocks
    var heightInBlocks = canvasHeight/blockSize;//divise la hauteur en terme de blocks donc 20 blocks
    var score;
    var timeout;
    ...
    function init()
    {
        var canvas = document.createElement('canvas'); // créer l'element html5 permettant de dessiner sur une page html
        ...
        document.body.appendChild(canvas); // permet d'accrocher ce tag au body
        ctx = canvas.getContext('2d');// dessine dans le canvas dans le contexte & attrape le contexte pour le mettre dans un nom
    ...
    function restart() // reinitialise le jeu à 0
    {
        snakee = new Snake([[6,4], [5,4], [4,4], [3,4], [2,4]], "right");//fonction constructor crée le body du serpent
        applee = new Apple([10,10]);//fonction constructor crée la pomme
        score = 0;
    ...
    {
        var x = position[0] * blockSize; // position à l'axe horizontal
        var y = position[1] * blockSize; // position à l'axe vertical
    ...
    function Snake(body, direction) // prototype du serpent
    {
        ...
        this.draw = function() // méthode dessinant le body du serpent
        {
            ...
            ctx.fillStyle = "#382a15"; // couleur du serpent
            for(var i = 0; i < this.body.length; i++) // boucle permettant de passer sur chacun des membres du serpent
            {
        ...
        {
            var nextPosition = this.body[0].slice();    //nouvelle position de la tête, on copie l'element [6,4] avec slice
        ...
        {
            var allowedDirections; // les directions permises
        ...
        this.checkCollision = function() //verifie si serpent sort du contexte ou passe sur son propre corps
    {
        var wallCollision = false; //initialise à faux
        var snakeCollision = false;//initialise à faux
        var head = this.body[0];//verifie la tete du serpent
        var rest = this.body.slice(1); //reste du corps du serpent mis de côté, est un array
        var snakeX = head[0];// x de la tete
        var snakeY = head[1];// y de la tete
        var minX = 0; //initialise à 0
        var minY = 0;//initialise à 0
        var maxX = widthInBlocks -1;//nombre de blocks -1
        var maxY = heightInBlocks -1;//nombre de blocks -1
        var isNotBetweenHorizontalWalls = snakeX < minX || snakeX > maxX //verifie si la tete entre les murs horizontales gauche ou droite
        var isNotBetweenVerticalWalls = snakeY < minY || snakeY > maxY //verifie si la tete entre les murs verticales haut ou bas
        ...
        for(var i = 0; i < rest.length ; i++) // verifie si serpent passe sur son propre corps
        {
        ...
        {
            var head = this.body[0];// verifie si la tete egale au corps à la place 0
    ...
    function Apple(position) // prototype de la pomme
    {
        ...
        {
            ctx.save();
            ctx.fillStyle = "#970c10";
            ctx.beginPath(); // il s'agit d'un rond
            var radius = blockSize/2; // r = diametre/2 definition du rayon
            var x = this.position[0]*blockSize + radius; // position du rond par rapport à l'horizontal
            var y = this.position[1]*blockSize + radius;// position du rond par rapport au vertical
        ...
        {
            var newX =  Math.round(Math.random() * (widthInBlocks - 1)); //multiplie par nombre de blocks dans la largeur -1 chiffre entre 0 & 29 avec chiffres entier
            var newY =  Math.round(Math.random() * (heightInBlocks - 1));
        this.position = [newX, newY];
        };
        this.isOnSnake = function(snakeToCheck) // verifie sur quel serpent se trouve la pomme
        { 
            var isOnSnake = false; //variable qu'on retourne à la fin (non c'est faux je ne suis pas sur le serpent)

        for(var i = 0 ; i < snakeToCheck.body.length; i++) // On va vérifier si pomme est sur chacun des blocks du serpent
        {
